...
prob1c.fun <- function(a,b){
  ifelse(a>b, return(b*factorial(a)), return(b-a))
}
```
...
duniform <-function(x,a,b){
  ifelse(x>=a & x<= b, return(1/(b-a)), return(0))
}
...
duniform <-function(x,a,b){
  ...
  for(index in 1:n)
  {
    ifelse(x[index]>=a & x[index]<= b, res[index] <- 1/(b-a), res[index] <- 0)
  }
